...
namespace OgrenciTakip.Bll.General
{
    ...
    {
        public IEnumerable<MesleklereGoreKayitRaporuList> List(Expression<Func<Tahakkuk, bool>> filter)
        {
            return BaseList(filter, x => new
            {
                ...
                OdemeBilgileri = x.OdemeBilgileri.GroupBy(y => y.TahakkukId).DefaultIfEmpty().Select(y => new
                {
                    ...
                    Tahsilde = y.Select(z => z.MakbuzHareketleri
                    .Where(c => c.BelgeDurumu == BelgeDurumu.AvukataGonderme || c.BelgeDurumu == BelgeDurumu.BankayaTahsileGonderme || c.BelgeDurumu == BelgeDurumu.CiroEtme || c.BelgeDurumu == BelgeDurumu.BlokeAlma)
                    .Select(c => c.IslemTutari).DefaultIfEmpty(0).Sum()).DefaultIfEmpty(0).Sum() -
                    y.Select(z => z.MakbuzHareketleri
            ...
            }).AsEnumerable().GroupBy(x => new { x.MeslekId, x.Tahakkuk.SubeId }).Select(x => new MesleklereGoreKayitRaporuList
            {
                SubeAdi = x.Select(y => y.Tahakkuk.Sube.SubeAdi).FirstOrDefault(),
                MeslekAdi = x.Select(y => y.MeslekAdi).FirstOrDefault() ?? "Meslek Girilmedi",
